...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # At the start, all corners are unvisited
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            #true : if wall in position; false: no wall in position
            if not hitsWall:
                nextState = (nextx, nexty)
                #visitedCorners tuple as a list
                xyCorners = list(state[1])
                #check if nextState is a corner
                if nextState in self.corners:
                    xyCorners[self.corners.index(nextState)] = True
                #new state
                xyState = (nextState, tuple(xyCorners))
                #add successor to list
                successors.append(((xyState), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    #mark unvisted corners
    unvisitedCorners = [corners[i] for i in range(4) if not visitedCorners[i]]
    for corner in unvisitedCorners:
        #find the distance from the currentstate and unvisted corner
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # BFS is able to find the most direct path 
